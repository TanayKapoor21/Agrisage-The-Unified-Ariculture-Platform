...
const products = [
  ...
  { id: 3, name: 'Premium Basmati Paddy Seeds', category: 'Seeds', price: 2100, seller: 'Royal Grains', rating: 4.9, img: 'https://images.unsplash.com/photo-1593113598332-cd288d649433?auto=format&fit=crop&w=150&h=150' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {products.map((p) => (
            <div key={p.id} className="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-800 p-4 flex gap-4 hover:shadow-md transition-shadow">
              <img src={p.img} alt={p.name} className="w-24 h-24 rounded-lg object-cover bg-gray-100 dark:bg-slate-800" referrerPolicy="no-referrer" />
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-800 p-5 space-y-4">
          ...
          <div className="space-y-4">
            {[
              { name: 'Sethi Fertilizers', dist: '1.2 km', rating: 4.9, loc: 'Goniana Road, Bathinda' },
              { name: 'Kisan Seva Kendra', dist: '3.5 km', rating: 4.7, loc: 'Malout Road, Bathinda' },
              { name: 'Bharat Seeds Corp', dist: '5.1 km', rating: 4.5, loc: 'Barnala Bypass, Bathinda' }
            ].map((shop, i) => (
              <div key={i} className="flex flex-col gap-1 border-b border-dashed border-gray-100 dark:border-slate-800 pb-3 last:border-0">
                ...
                <div className="flex items-center gap-1 text-[10px] text-gray-500 dark:text-slate-500">
                  <MapPin className="w-3 h-3" /> {shop.loc}
                </div>
